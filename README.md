...
欢迎同学们修 bug、加功能！本项目采用标准的 **Fork & Pull Request** 流程,不需要直接的 commit 权限。

### 一次性准备(每个贡献者只做一次)

1. 在 GitHub 上访问 [本项目](https://github.com/EggplantOne/SAMannot-DPG),点击右上角 **Fork** 按钮,把项目复制一份到你自己的账号下(例如 `张三/SAMannot-DPG`)
2. 把你 fork 的仓库 clone 到本地:
   ```bash
   git clone https://github.com/<你的用户名>/SAMannot-DPG.git
   cd SAMannot-DPG
   ```
3. 把原项目添加为 upstream(以后同步用):
   ```bash
   git remote add upstream https://github.com/EggplantOne/SAMannot-DPG.git
   ```
...
1. **同步主仓库最新代码**(每次开始改之前):
   ...
   ```
2. **从最新 main 拉一个新分支**(分支名能描述改动即可):
   ```bash
   git checkout -b fix/wrong-mask-color upstream/main
   ```
   - bug 修复:`fix/<简短描述>`
   - 新功能:`feat/<简短描述>`
   - 重构:`refactor/<简短描述>`
3. **改代码 → 测试 → 提交**:
   ```bash
   git add <改动的文件>
   git commit -m "fix: 描述这个 bug 的修复"
   ```
   一个 PR 只解决一件事,如果发现另一个 bug,新开一个分支再提一个 PR。
4. **更新 CHANGELOG.md**:在文件顶部加一条说明你的改动(参考已有条目的写法)。
5. **推到自己的 fork**:
   ```bash
   git push origin fix/wrong-mask-color
   ```
6. **在 GitHub 网页上开 PR**:
   - 推完之后 GitHub 会自动提示 "Compare & pull request",点进去
   - PR 模板会自动填好(`.github/pull_request_template.md`),按提示写说明、勾 checklist
   - 如果有相关 issue 编号,在描述里写 `Closes #<编号>` 可以自动关联

### 代码规范

- **不要碰 `projects/`**:这个目录是用户的私人标注数据,`.gitignore` 已经排除,不要 `git add` 进去
- **不要碰 `checkpoints/`**:模型权重文件几百 MB,不要提交
- **测试基本路径**:至少跑一次 `python main.py` 确认能启动、加载帧、添加 label、点几下 prompts、Single 推理无报错
- **改了批量编辑(reassign / delete-in-range / reconcile)**:手动验证一下 JSON 和 pkl 都正确落盘
- **commit 消息**:简短中英文都可,推荐前缀 `fix:` / `feat:` / `refactor:` / `docs:`

### 报 Bug 而不是直接修?

也欢迎!如果你不确定怎么修、或者想先讨论方案,可以去 [Issues](https://github.com/EggplantOne/SAMannot-DPG/issues) 开一个新 issue。模板会引导你填写复现步骤、环境信息等。
